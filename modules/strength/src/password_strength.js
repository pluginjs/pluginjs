/* Credit to https://github.com/fnando/password_strength/blob/master/app/assets/javascripts/password_strength.js MIT */

const MULTIPLE_NUMBERS_RE = /\d.*?\d.*?\d/
const MULTIPLE_SYMBOLS_RE = /[!@#$%^&*?_~].*?[!@#$%^&*?_~]/
/* eslint-disable */
const UPPERCASE_LOWERCASE_RE = /([a-z].*[A-Z])|([A-Z].*[a-z])/
const SYMBOL_RE = /[!@#\$%^&*?_~]/
...
export class PasswordStrength {
  ...
  containInvalidRepetition() {
    const char = this.password[0]

    if (!char) {
      return
    }

    const regex = new RegExp(`^${escapeForRegexp(char)}+$`, 'i')

    return regex.test(this.password)
  }

  usesCommonWord() {
    return commonWords.includes(this.password.toLowerCase())
  }
